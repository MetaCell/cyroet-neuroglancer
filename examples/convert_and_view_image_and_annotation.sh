...
cryoet-converter create-image $RELATIVE_TOMOGRAM_PATH -z "$LOCAL_DATA_STORE_PATH/$RELATIVE_TOMOGRAM_PATH" -u $REMOTE_DATA_STORE_URL -r 1.348 -n "TS_026 tomogram" -o "$LOCAL_DATA_STORE_PATH/TS_026_tomogram.json"
...
cryoet-converter create-annotation TS_026_ribosome -z "$LOCAL_DATA_STORE_PATH/$RELATIVE_FA_ANN_PATH" -u $REMOTE_DATA_STORE_URL -n "TS_026 ribosome" -o "$LOCAL_DATA_STORE_PATH/TS_026_ribosome.json" -c "#ff0000 red" -s 0.2
...
cryoet-converter create-annotation TS_026_fatty_acid -z "$LOCAL_DATA_STORE_PATH/$RELATIVE_FA_ANN_PATH" -u $REMOTE_DATA_STORE_URL -n "TS_026 fatty acid" -o "$LOCAL_DATA_STORE_PATH/TS_026_fatty_acid.json" -c "#0000ff blue" -s 0.2
